...
export async function allTicketPasses() {
  ...
  const rows = await query(
    `SELECT id, ticket_tier, guests, currency_code, ticket_price_indicative, effective_price_per_guest, status, created_at, updated_at
...
export function toTicketTypesMap(ticketPasses) {
  return ticketPasses.reduce((ticketTypes, ticketPass) => {
    ...
    ticketTypes[String(ticketPass.id)] = {
      ...
      note: `${ticketPass.guests} / ${ticketPass.currency_code || 'USD'} ${formatNumber(ticketPass.effective_price_per_guest)} per guest`,
    };
    return ticketTypes;
  }, {});
}
...
export async function createTicketPass(body) {
  ...
  const result = await query(
    `INSERT INTO ticket_passes_tb (ticket_tier, guests, currency_code, ticket_price_indicative, effective_price_per_guest, status)
     VALUES (?, ?, ?, ?, ?, 1)
     ON DUPLICATE KEY UPDATE
       ...
       status = 1`,
    [ticketPass.ticket_tier, ticketPass.guests, ticketPass.currency_code, ticketPass.ticket_price_indicative, ticketPass.effective_price_per_guest],
  );
...
export async function updateTicketPass(id, body) {
  ...
  await query(
    `UPDATE ticket_passes_tb
     SET ticket_tier = ?, guests = ?, currency_code = ?, ticket_price_indicative = ?, effective_price_per_guest = ?
     WHERE id = ?`,
    [ticketPass.ticket_tier, ticketPass.guests, ticketPass.currency_code, ticketPass.ticket_price_indicative, ticketPass.effective_price_per_guest, id],
  );
  return ticketPass;
}
...
function normalizePasses(ticketPasses) {
  return ticketPasses.map((ticketPass) => {
    ...
    return {
      ...
      guests,
      currency_code: normalizeCurrency(ticketPass.currency_code),
      ticket_price_indicative: ticketPriceIndicative,
...
function normalizeCurrency(value) {
  ...
  return ['USD', 'LKR'].includes(currency) ? currency : 'USD';
}
